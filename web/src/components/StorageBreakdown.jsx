...
export function StorageBreakdown({ byMimeType, usedBytes, isLoading }) {
  ...
  const buckets = byMimeType ? bucketByCategory(byMimeType) : [];
  const total = byMimeType ? Number(usedBytes) || buckets.reduce((sum, b) => sum + b.sizeBytes, 0) : 0;

  return (
    <AnimatePresence mode="wait" initial={false}>
      {isLoading || !byMimeType ? (
        <motion.div
          key="loading"
          ...
          exit="exit"
          transition={quick}
          className="h-[132px] animate-pulse rounded-lg border border-edge bg-surface"
        />
      ) : total === 0 ? (
        <motion.div
          key="empty"
          ...
          exit="exit"
          transition={quick}
          className="rounded-lg border border-edge bg-surface p-4 text-sm text-muted"
        >
